...
[E,V] = eig(cov(feat'));
PCA_transform = E(:,max(1, end-dimUsed+1):end)';
featDecorrelated = PCA_transform * feat;

std_dev = std(featDecorrelated');
idx = find(std_dev==0);
std_dev(idx) = 1;
W = diag(1./std_dev) * PCA_transform;
featDecorrelated2 = W * feat;
b = -mean(featDecorrelated2,2);
