...
%
%clear
function TrainSeparationNet_Regression(hiddenLayerSize, hiddenLayerSizeFF, learning_rate, nUtt4Iteration, SPR, seglen, useGPU)
modelType = 'LSTM';
...
para.local.seglen = seglen;
para.local.segshift = seglen;
...
[layer, para] = Build_SeparationNet_Regression(Data_tr, para);
...
if para.topology.useCMN; para.output = 'nnet/SepReg.CMN';
else;     para.output = 'nnet/SepReg.noCMN';    end
para.output = sprintf('%s.%s.MbSize%d.SPR%d.seg%d.U%d.%d-%s', para.output, para.topology.DeltaGenerationType, ...
    para.NET.nSequencePerMinibatch, SPR, seglen, length(Data_tr(1).data),  3*(para.topology.fft_len/2+1), para.topology.RegressionNetType);
