...
idx = ReturnLayerIdxByName(layer, 'log');
fft_net_length = idx(1);
fft_net = layer(1:fft_net_length);
paraTmp = para;
paraTmp.out_layer_idx = fft_net_length;
paraTmp.IO = RemoveIOStream(paraTmp.IO, [2 3]);
paraTmp.IO.nStream = 1;
paraTmp.IO.inputFeature = paraTmp.IO.DynamicMixture.inputFeature(1);
if isfield(paraTmp.IO, 'fileReader'); paraTmp.IO = rmfield(paraTmp.IO, 'fileReader'); end
paraTmp.IO.fileReader(1)= paraTmp.IO.DynamicMixture.fileReader(1);
...
for i=1:length(weight_idx)
    ...
    if layer{weight_idx(i)}.dim(1) == size(layer{weight_idx(i)}.W,1)*2
        layer{weight_idx(i)}.W = diag(repmat(diag(layer{weight_idx(i)}.W),2,1));
        layer{weight_idx(i)}.b = [layer{weight_idx(i)}.b; layer{weight_idx(i)}.b];
    end
end
