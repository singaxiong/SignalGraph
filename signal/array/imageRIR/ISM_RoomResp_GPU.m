function [RIRvec] = ISM_RoomResp_GPU(Fs,beta,rt_type,rt_val,X_src,X_rcv,room,varargin)
...
for a = 0:1
    for b = 0:1
        for d = 0:1
            ...
            [FoundLValBelowLim, tmp_time, tmp_amp] = Check_lDim(a,b,d,m,X_rcv,X_src,Rr,c,MaxDelay,beta,Fs);
            foo_time2 = [foo_time2 tmp_time];
            foo_amplitude2 = [foo_amplitude2 tmp_amp];
...
if length(foo_time2)>0
    if useGPU
        TimePoints = gpuArray(TimePoints);
        maxBlkSize = round(1e7/length(TimePoints));
        for i=1:maxBlkSize:length(foo_time2)
            idx = min(i+maxBlkSize-1, length(foo_time2));
            tmp = sinc(bsxfun(@minus, TimePoints, foo_time2(i:idx))*Fs);
            RIRvec = RIRvec + tmp * foo_amplitude2(i:idx)';
        end
        RIRvec = gather(RIRvec);
    else
        if 1    % for large source to mic distance, we may need to break into a lot of small blocks, otherwise the memory consumption is too big
%             tic;
            maxBlkSize = round(5e7/length(TimePoints));
            for i=1:maxBlkSize:length(foo_time2)
                idx = min(i+maxBlkSize-1, length(foo_time2));
                tmp = sinc(bsxfun(@minus, TimePoints, foo_time2(i:idx))*Fs);
                RIRvec = RIRvec + tmp * foo_amplitude2(i:idx)';
...
%             toc;
            ...
            tmp = sinc(bsxfun(@minus, TimePoints, foo_time2)*Fs);
            RIRvec = tmp * foo_amplitude2';
        end
    end
end

%============
function [FoundLValBelowLim, foo_time2, foo_amplitude2] = Check_lDim(a,b,d,m,X_rcv,X_src,Rr,c,MaxDelay,beta,Fs)
foo_time2 = [];
foo_amplitude2 = [];
...
[FoundNValBelowLim, tmp_time, tmp_amp] = Check_nDim_fast(a,b,d,l,m,X_rcv,X_src,Rr,c,MaxDelay,beta,Fs);
foo_time2 = [foo_time2 tmp_time];
foo_amplitude2 = [foo_amplitude2 tmp_amp];
...
%============
function [FoundNValBelowLim, foo_time2, foo_amplitude2] = Check_nDim_fast(a,b,d,l,m,X_rcv,X_src,Rr,c,MaxDelay,beta,Fs)
foo_amplitude2 = [];foo_time2 = [];
...
while foo_time<=MaxDelay,    % if delay is below TF length limit for n=1, check n=2,3,4...
    ...
    foo_amplitude2(end+1) = foo_amplitude;
    foo_time2(end+1) = foo_time;
    n = n+1;
    dist = norm( [2*a-1; 2*b-1; 2*d-1].*X_src + X_rcv - Rr.*[n;l;m] );
    foo_time = dist/c;
end
