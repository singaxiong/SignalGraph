 function [OutStruct] = imsetup_phase3(t60,room,distance,angle)
...
switch upper(room)
    case 'SMALL'
        OutStruct.roomDimension = [6.00 4.00 3.00];           % room dimensions in m
    case'MEDIUM'
        OutStruct.roomDimension = [10.00 8.00 3.00];           % room dimensions in m    
    case 'LARGE'
        OutStruct.roomDimension = [14.00 12.00 3.00];       % original [15,10,3]   % room dimensions in m    
...
switch upper(distance)
    case 'FAR'
        if isequal(room,'small'), 
            R=1.5;  % distance between the source and the microphone array
