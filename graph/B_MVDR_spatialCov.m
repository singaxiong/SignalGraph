...
if N>1
    ...
    for i=1:N
        grad{i} = GetGradUtt(reshape(XUtt{i},D,C,size(XUtt{i},2)),YUtt{i},phi_s(:,:,:,:,i), phi_n(:,:,:,:,i), ...
            weight(:,:,i), lambda(:,:,:,:,i), future_gradUtt{i});
    end
    grad = cell2mat_gpu(grad);
    grad = permute(grad, [1 3 2]);
else
    grad = GetGradUtt(X,Y,phi_s, phi_n, weight, lambda, future_grad);
end
...
function grad = GetGradUttVec(X,Y,phi_s, phi_n, weight, lambda, future_grad)
[D,C,T,N] = size(X);
u = zeros(C,1);
u(1) = 1;
...
function grad = GetGradUtt(X,Y,phi_s, phi_n, weight, lambda, future_grad)
[D,C,T,N] = size(X);
u = zeros(C,1);
u(1) = 1;
for f=1:D
